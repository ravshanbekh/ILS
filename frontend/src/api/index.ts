...
export const liveQuizApi = {
  ...
  startQuiz: (id: string, data?: { musicId?: string; groupId?: string }) => api.patch(`/live-quiz/${id}/start`, data),     // Yangi kod + lobby
  ...
  nextQuestion: (id: string, questionId?: string) => api.patch(`/live-quiz/${id}/next`, { questionId }),   // Leaderboard ko'rsatish (savol o'tmaydi)
  finishQuiz: (id: string) => api.patch(`/live-quiz/${id}/finish`),
  getStats: (id: string) => api.get(`/live-quiz/${id}/stats`),
  getResults: () => api.get('/live-quiz/results/all'),
  ...
  rejoinQuiz: (code: string, playerId: string | null, fullName: string) =>
    api.post(`/live-quiz/join/${code}/rejoin`, { playerId, fullName }),
